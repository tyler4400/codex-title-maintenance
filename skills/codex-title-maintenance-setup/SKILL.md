...
- 默认纳入归档及实际支持的本机来源，排除内部子任务和用户显式排除项。heartbeat 当前绑定的固定维护任务始终排除；cron 当前运行任务通过实时状态暂缓，未被显式排除的已结束旧轮次可由后续增量处理。
...
用户要求 `start` 时，已有绑定或明确选择就沿用，只调整用户要求的部分。没有绑定且尚未选择时，按用户诉求说明取舍，不把任何一种模式作为统一推荐；仅初始化或改配置时不必创建计划。

| 用户更在意什么 | 适合的运行方式与取舍 |
|---|---|
| 减少维护对话数量 | heartbeat：复用一个专用维护任务，上下文会积累；可选择精简输出，必要时按操作流程换绑 |
| 每轮独立执行 | cron / New chat each run：绑定项目，每轮从保存的提示启动新任务，也会持续增加运行对话 |
| 保留已有运行习惯 | 沿用当前绑定，不因重新引导而更换模式、目标或模型 |

提问时把符合已知诉求的选项放在 `a)`，并说明推荐理由；诉求不明时，先让用户选择更在意对话数量还是每轮独立。是否精简输出、何时轮换及是否归档旧任务都属于用户选择，不自动设定周期或清理旧对话。

选择 cron 时通过 `list_projects` 或当前工具的实际读回取得项目 ID，不能从名称、cwd 或路径猜测；还需使用 `executionEnvironment=local` 和原生 automation 支持的具体 model / reasoning。选择 heartbeat 时才要求用户选定固定维护任务。执行、换绑及可选精简输出的细节交给主 skill 的 [operations.md](../codex-title-maintenance/references/operations.md)。
